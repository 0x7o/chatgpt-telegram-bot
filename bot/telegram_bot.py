...
def model_keyboard(default_model: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    text="✅ GPT-4o-mini" if default_model == "gpt35" else "GPT-4o-mini",
...
"""
    ...
    async def stats(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        text_budget += (
            f"<b>Токенов GPT-4o-mini осталось:</b> {user.gpt35_rate} из {self.rates[user.rate_type]['gpt35_rate']}\n"
    ...
    async def sdxl_prompt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def sdxl_negative_prompt(
        ...
        return self.SDXL_SEED

    async def sdxl_seed(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        user, okay = await self.check_rate_limit(
            update, update.effective_chat.id, "dalle_rate"
        )
    ...
    async def sticker_photo(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        photo = update.message.photo[-1]
        photo_file = await context.bot.get_file(photo.file_id)
        photo_url = photo_file.file_path
        context.user_data["photo_url"] = photo_url
    ...
    async def sticker_prompt(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        user, okay = await self.check_rate_limit(
            update, update.effective_chat.id, "dalle_rate"
        )
    ...
    async def pay(self, update: Update, _: ContextTypes.DEFAULT_TYPE):
        ...
        rates_text = [
            (
                ...
                f"Токенов GPT-4o-mini: {rate['gpt35_rate']}\n"
...
Высокой продуктивности, {update.callback_query.from_user.first_name}, на связи 👋"""
    ...
    async def cancel(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        await update.message.reply_text(
            message_thread_id=get_thread_id(update), text="❌ Отмена"
        )
        return ConversationHandler.END
    ...
    def run(self):
        ...
        application.add_handler(
            ConversationHandler(
                entry_points=[CommandHandler("sdxl", self.sdxl)],
                states={
                    self.SDXL_PROMPT: [
                        MessageHandler(
                            filters.TEXT & ~filters.COMMAND, self.sdxl_prompt
                        )
                    ],
                    self.SDXL_NEGATIVE_PROMPT: [
                        MessageHandler(
                            filters.TEXT & ~filters.COMMAND, self.sdxl_negative_prompt
                        )
                    ],
                    self.SDXL_SEED: [
                        MessageHandler(filters.TEXT & ~filters.COMMAND, self.sdxl_seed)
                    ],
                },
